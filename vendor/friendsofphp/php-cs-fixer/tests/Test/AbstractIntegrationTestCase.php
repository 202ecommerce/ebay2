...
use EbayVendor\PhpCsFixer\Error\Error;
use EbayVendor\PhpCsFixer\Error\ErrorsManager;
use EbayVendor\PhpCsFixer\FileRemoval;
use EbayVendor\PhpCsFixer\Fixer\FixerInterface;
use EbayVendor\PhpCsFixer\FixerFactory;
use EbayVendor\PhpCsFixer\Linter\CachingLinter;
use EbayVendor\PhpCsFixer\Linter\Linter;
use EbayVendor\PhpCsFixer\Linter\LinterInterface;
use EbayVendor\PhpCsFixer\Linter\ProcessLinter;
use EbayVendor\PhpCsFixer\Runner\Runner;
use EbayVendor\PhpCsFixer\Tests\TestCase;
use EbayVendor\PhpCsFixer\Tokenizer\Tokens;
use EbayVendor\PhpCsFixer\WhitespacesFixerConfig;
use EbayVendor\Prophecy\Argument;
use EbayVendor\Symfony\Component\Filesystem\Exception\IOException;
...
abstract class AbstractIntegrationTestCase extends TestCase
{
    ...
    protected function doTest(IntegrationCase $case)
    {
        ...
        $this->testIntegration(new IntegrationCase($case->getFileName(), $case->getTitle() . ' "--EXPECT-- part run"', $case->getSettings(), $case->getRequirements(), $case->getConfig(), $case->getRuleset(), $case->getExpectedCode(), null));
    }
    /**
    ...
    private function getLinter()
    {
        ...
        if (null === $linter) {
            if (\getenv('SKIP_LINT_TEST_CASES')) {
                $linterProphecy = $this->prophesize(\EbayVendor\PhpCsFixer\Linter\LinterInterface::class);
                $linterProphecy->lintSource(Argument::type('string'))->willReturn($this->prophesize(\EbayVendor\PhpCsFixer\Linter\LintingResultInterface::class)->reveal());
                $linterProphecy->lintFile(Argument::type('string'))->willReturn($this->prophesize(\EbayVendor\PhpCsFixer\Linter\LintingResultInterface::class)->reveal());
