 ...
 */
namespace EbayVendor\PhpCsFixer\Console;

use EbayVendor\PhpCsFixer\Cache\CacheManagerInterface;
use EbayVendor\PhpCsFixer\Cache\Directory;
use EbayVendor\PhpCsFixer\Cache\DirectoryInterface;
use EbayVendor\PhpCsFixer\Cache\FileCacheManager;
use EbayVendor\PhpCsFixer\Cache\FileHandler;
use EbayVendor\PhpCsFixer\Cache\NullCacheManager;
use EbayVendor\PhpCsFixer\Cache\Signature;
use EbayVendor\PhpCsFixer\ConfigInterface;
use EbayVendor\PhpCsFixer\ConfigurationException\InvalidConfigurationException;
use EbayVendor\PhpCsFixer\Console\Report\FixReport\ReporterFactory;
use EbayVendor\PhpCsFixer\Console\Report\FixReport\ReporterInterface;
use EbayVendor\PhpCsFixer\Differ\DifferInterface;
use EbayVendor\PhpCsFixer\Differ\NullDiffer;
use EbayVendor\PhpCsFixer\Differ\SebastianBergmannDiffer;
use EbayVendor\PhpCsFixer\Differ\UnifiedDiffer;
use EbayVendor\PhpCsFixer\Finder;
...
use EbayVendor\PhpCsFixer\Linter\Linter;
use EbayVendor\PhpCsFixer\Linter\LinterInterface;
use EbayVendor\PhpCsFixer\RuleSet\RuleSet;
use EbayVendor\PhpCsFixer\StdinFileInfo;
use EbayVendor\PhpCsFixer\ToolInfoInterface;
use EbayVendor\PhpCsFixer\Utils;
use EbayVendor\PhpCsFixer\WhitespacesFixerConfig;
...
use EbayVendor\Symfony\Component\Finder\Finder as SymfonyFinder;
/**
...
final class ConfigurationResolver
{
        ...
        if (null === $this->progress) {
            if (OutputInterface::VERBOSITY_VERBOSE <= $this->options['verbosity'] && 'txt' === $this->getFormat()) {
                ...
                if (null === $progressType) {
                    ...
                    Utils::triggerDeprecation(new \InvalidArgumentException('Passing `estimating`, `estimating-max` or `run-in` is deprecated and will not be supported in 3.0, use `none` or `dots` instead.'));
                }
    ...
    private function resolveOptionBooleanValue($optionName)
    {
        ...
        Utils::triggerDeprecation(new InvalidConfigurationException(\sprintf('Expected "yes" or "no" for option "%s", other values are deprecated and support will be removed in 3.0. Got "%s", this implicitly set the option to "false".', $optionName, $value)));
        return \false;
    }
