 ...
 */
namespace EbayVendor\PhpCsFixer\FixerConfiguration;
...
final class FixerConfigurationResolverRootless implements FixerConfigurationResolverInterface
{
    ...
    public function __construct($root, $options, $fixerName)
    {
        ...
        $names = \array_map(static function (FixerOptionInterface $option) {
    ...
    public function resolve(array $options)
    {
        if (!empty($options) && !\array_key_exists($this->root, $options)) {
            ...
            if (!empty(\array_diff($passedNames, $names))) {
                Utils::triggerDeprecation(new \RuntimeException("Passing \"{$this->root}\" at the root of the configuration for rule \"{$this->fixerName}\" is deprecated and will not be supported in 3.0, use \"{$this->root}\" => array(...) option instead."));
