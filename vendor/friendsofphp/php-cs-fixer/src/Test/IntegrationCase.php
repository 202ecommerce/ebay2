 ...
 */
namespace EbayVendor\PhpCsFixer\Test;

use EbayVendor\PhpCsFixer\RuleSet\RuleSet;
use EbayVendor\PhpCsFixer\Tests\Test\IntegrationCase as BaseIntegrationCase;
...
final class IntegrationCase
{
    ...
    public function __construct($fileName, $title, array $settings, array $requirements, array $config, RuleSet $ruleset, $expectedCode, $inputCode)
    {
        ...
        Utils::triggerDeprecation(new \RuntimeException(\sprintf('The "%s" class is deprecated. You should stop using it, as it will be removed in 3.0 version.', __CLASS__)));
    }
    ...
    {
        Utils::triggerDeprecation(new \RuntimeException(\sprintf('The "%s" method is deprecated. You should stop using it, as it will be removed in 3.0 version.', __METHOD__)));
