...
use EbayVendor\Symfony\Component\OptionsResolver\Options;
/**
...
', ['elements' => ['const' => self::SPACING_ONE]])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens)
    {
        ...
        foreach (\array_reverse($tokensAnalyzer->getClassyElements(\true), \true) as $index => $element) {
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('elements', 'Dictionary of `const|method|property|trait_import` => `none|one` values.'))->setNormalizer(static function (Options $options, $values) {
            $deprecated = \array_intersect($values, self::SUPPORTED_TYPES);
            if (\count($deprecated) > 0) {
                Utils::triggerDeprecation(new \RuntimeException('A list of elements is deprecated, use a dictionary of `const|method|property` => `none|one` instead.'));
