...
use EbayVendor\Symfony\Component\Console\Helper\SymfonyQuestionHelper;
use EbayVendor\Symfony\Component\Console\Output\StreamOutput;
use EbayVendor\Symfony\Component\Console\Question\ChoiceQuestion;
use EbayVendor\Symfony\Component\Console\Question\Question;
/**
 * @group tty
 */
class SymfonyQuestionHelperTest extends AbstractQuestionHelperTest
{
    ...
    {
        $helper = new SymfonyQuestionHelper();
        $input = $this->createStreamableInputInterfaceMock($this->getInputStream('EbayVendor\\Foo\\Bar'));
        $helper->ask($input, $output = $this->createOutputInterface(), new Question('Do you want to use Foo\\Bar <comment>or</comment> Foo\\Baz\\?', 'EbayVendor\\Foo\\Baz'));
    ...
    {
        $this->expectException('EbayVendor\\Symfony\\Component\\Console\\Exception\\RuntimeException');
    ...
    {
        $choiceQuestion = new ChoiceQuestion('qqq', ['foo' => 'foo', 'żółw' => 'bar', 'łabądź' => 'baz']);
        (new SymfonyQuestionHelper())->ask($this->createStreamableInputInterfaceMock($this->getInputStream("foo\n")), $output = $this->createOutputInterface(), $choiceQuestion);
...
, $output, \true);
    ...
    {
        $mock = $this->getMockBuilder('EbayVendor\\Symfony\\Component\\Console\\Input\\InputInterface')->getMock();
