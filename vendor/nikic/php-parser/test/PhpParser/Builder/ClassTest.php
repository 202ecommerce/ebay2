...
class ClassTest extends \EbayVendor\PHPUnit_Framework_TestCase
{
    ...
    {
        $node = $this->createClassBuilder('SomeLogger')->extend('BaseLogger')->implement('EbayVendor\\Namespaced\\Logger', new Name('SomeInterface'))->implement('EbayVendor\\Fully\\Qualified', 'EbayVendor\\namespace\\NamespaceRelative')->getNode();
        $this->assertEquals(new Stmt\Class_('SomeLogger', array('extends' => new Name('BaseLogger'), 'implements' => array(new Name('EbayVendor\\Namespaced\\Logger'), new Name('SomeInterface'), new Name\FullyQualified('EbayVendor\\Fully\\Qualified'), new Name\Relative('NamespaceRelative')))), $node);
    }
    public function testAbstract()
    {
        $node = $this->createClassBuilder('Test')->makeAbstract()->getNode();
        $this->assertEquals(new Stmt\Class_('Test', array('flags' => Stmt\Class_::MODIFIER_ABSTRACT)), $node);
    }
    public function testFinal()
    {
        $node = $this->createClassBuilder('Test')->makeFinal()->getNode();
        $this->assertEquals(new Stmt\Class_('Test', array('flags' => Stmt\Class_::MODIFIER_FINAL)), $node);
    }
...
DOC;
        ...
        $this->assertEquals(new Stmt\Class_('Test', array(), array('comments' => array(new Comment\Doc($docComment)))), $class);
