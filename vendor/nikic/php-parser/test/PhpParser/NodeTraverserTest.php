...
use EbayVendor\PhpParser\Node\Scalar\String_;
class NodeTraverserTest extends \EbayVendor\PHPUnit_Framework_TestCase
{
    public function testNonModifying()
    {
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
    ...
    public function testModifying()
    {
        ...
        $visitor1 = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor2 = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
    ...
    public function testStopTraversal()
    {
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor->expects($this->at(1))->method('enterNode')->with($mulNode)->will($this->returnValue(NodeTraverser::STOP_TRAVERSAL));
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor->expects($this->at(2))->method('enterNode')->with($varNode1)->will($this->returnValue(NodeTraverser::STOP_TRAVERSAL));
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor->expects($this->at(3))->method('leaveNode')->with($varNode1)->will($this->returnValue(NodeTraverser::STOP_TRAVERSAL));
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor->expects($this->at(6))->method('leaveNode')->with($mulNode)->will($this->returnValue(NodeTraverser::STOP_TRAVERSAL));
        ...
        $visitor = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor->expects($this->at(6))->method('leaveNode')->with($mulNode)->will($this->returnValue(NodeTraverser::REMOVE_NODE));
        $visitor->expects($this->at(7))->method('enterNode')->with($printNode)->will($this->returnValue(NodeTraverser::STOP_TRAVERSAL));
    ...
    public function testRemovingVisitor()
    {
        $visitor1 = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor2 = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $visitor3 = $this->getMockBuilder('EbayVendor\\PhpParser\\NodeVisitor')->getMock();
        $traverser = new NodeTraverser();
        $traverser->addVisitor($visitor1);
        $traverser->addVisitor($visitor2);
