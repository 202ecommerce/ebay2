...
namespace EbayVendor\PhpParser\NodeVisitor;
...
class NameResolverTest extends \EbayVendor\PHPUnit_Framework_TestCase
{
...
namespace EbayVendor\Foo;

use EbayVendor\Hallo as Hi;
new Bar();
new Hi();
new Hi\Bar();
new \EbayVendor\Bar();
...
\EbayVendor\bar();
namespace\bar();
namespace EbayVendor\Bar;

use function EbayVendor\foo\bar as baz;
use const EbayVendor\foo\BAR as BAZ;
use EbayVendor\foo as bar;
bar();
baz();
bar\foo();
baz\foo();
...
BAR\FOO;
BAZ\FOO;
namespace EbayVendor\Baz;

use EbayVendor\A\T\B\C;
use EbayVendor\A\T\D\E;
use function EbayVendor\X\T\b\c;
use function EbayVendor\X\T\d\e;
use const EbayVendor\Y\T\B\C;
use const EbayVendor\Y\T\D\E;
use EbayVendor\Z\T\G;
use function EbayVendor\Z\T\f;
use const EbayVendor\Z\T\K;
new C();
new E();
new C\D();
new E\F();
...
EOC;
    ...
    {
        $nsStmts = array(new Stmt\Class_('A'), new Stmt\Interface_('B'), new Stmt\Function_('C'), new Stmt\Const_(array(new Node\Const_('D', new Node\Scalar\LNumber(42)))), new Stmt\Trait_('E'), new Expr\New_(new Stmt\Class_(null)));
        $traverser = new PhpParser\NodeTraverser();
        $traverser->addVisitor(new NameResolver());
        $stmts = $traverser->traverse([new Stmt\Namespace_(new Name('NS'), $nsStmts)]);
        $this->assertSame('EbayVendor\\NS\\A', (string) $stmts[0]->stmts[0]->namespacedName);
        $this->assertSame('EbayVendor\\NS\\B', (string) $stmts[0]->stmts[1]->namespacedName);
        $this->assertSame('EbayVendor\\NS\\C', (string) $stmts[0]->stmts[2]->namespacedName);
        $this->assertSame('EbayVendor\\NS\\D', (string) $stmts[0]->stmts[3]->consts[0]->namespacedName);
        $this->assertSame('EbayVendor\\NS\\E', (string) $stmts[0]->stmts[4]->namespacedName);
    ...
    {
        $stmts = array(new Stmt\Namespace_(new Name('NS'), array(new Expr\FuncCall(new Name('foo')), new Expr\ConstFetch(new Name('FOO')))), new Stmt\Namespace_(null, array(new Expr\FuncCall(new Name('foo')), new Expr\ConstFetch(new Name('FOO')))));
        $traverser = new PhpParser\NodeTraverser();
        $traverser->addVisitor(new NameResolver());
        $stmts = $traverser->traverse($stmts);
        $this->assertSame('EbayVendor\\NS\\foo', (string) $stmts[0]->stmts[0]->name->getAttribute('namespacedName'));
        $this->assertSame('EbayVendor\\NS\\FOO', (string) $stmts[0]->stmts[1]->name->getAttribute('namespacedName'));
    ...
    {
        $this->setExpectedException('EbayVendor\\PhpParser\\Error', $errorMsg);
        $traverser = new PhpParser\NodeTraverser();
        $traverser->addVisitor(new NameResolver());
...
EOC;
    ...
    {
        $traverser = new PhpParser\NodeTraverser();
        $traverser->addVisitor(new NameResolver(null, ['preserveOriginalNames' => \true]));
        $n1 = new Name('Bar');
        $n2 = new Name('bar');
        $origStmts = [new Stmt\Namespace_(new Name('Foo'), [new Expr\ClassConstFetch($n1, 'FOO'), new Expr\FuncCall($n2)])];
