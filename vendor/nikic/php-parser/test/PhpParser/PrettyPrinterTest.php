...
use EbayVendor\PhpParser\Node\Scalar\Encapsed;
use EbayVendor\PhpParser\Node\Scalar\EncapsedStringPart;
...
use EbayVendor\PhpParser\PrettyPrinter\Standard;
...
class PrettyPrinterTest extends CodeTestAbstract
{
    ...
    {
        $prettyPrinter = new Standard(['shortArraySyntax' => \true]);
        $expr = new Expr\Array_([new Expr\ArrayItem(new String_('val'), new String_('key'))]);
        $expected = "['key' => 'val']";
    ...
    {
        return [[new LNumber(-1), '-1'], [new LNumber(-\PHP_INT_MAX - 1), '(-' . \PHP_INT_MAX . '-1)'], [new LNumber(-1, ['kind' => LNumber::KIND_BIN]), '-0b1'], [new LNumber(-1, ['kind' => LNumber::KIND_OCT]), '-01'], [new LNumber(-1, ['kind' => LNumber::KIND_HEX]), '-0x1'], [new DNumber(\INF), '\\INF'], [new DNumber(-\INF), '-\\INF'], [new DNumber(-\NAN), '\\NAN']];
    }
    /**
