...
class EbayTaskManager
{
    ...
    public static function getErrors($id_ebay_profile) {
        ...
        return DB::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'ebay_task_manager WHERE `error` IS NOT NULL and `id_ebay_profile` = '.(int)$id_ebay_profile.' ORDER BY `date_created`');
    }
}