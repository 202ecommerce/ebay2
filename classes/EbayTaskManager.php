    ...
    public static function toJob()
    {
        ...
        if ($tasks) {
            while ($row = $tasks->fetch(PDO::FETCH_ASSOC)) {
                ...
                $response = EbaySynchronizer::callFunction(self::getMethode($row['id_task']), ['id_product' => $row['id_product'], 'id_product_attribute' => $row['id_product_attribute'], 'id_ebay_profile' => $row['id_ebay_profile']]);
