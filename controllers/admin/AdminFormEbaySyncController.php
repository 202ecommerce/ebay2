...
class AdminFormEbaySyncController extends ModuleAdminController
{
    ...
    public function ajaxProcessSaveConfigFormCategory()
    {
        ...
        if (($all_nb_extra_images = Tools::getValue('all-extra-images-value', -1)) != -1) {
            ...
            foreach ($product_ids as $product_id) {
                EbayProductConfiguration::insertOrUpdate($product_id, [
        ...
        if (Tools::getValue('specific')) {
            foreach (Tools::getValue('specific') as $specific_id => $data) {
                ...
                $data = array_combine(array_values($field_names), [null, null, null, null, null, null, null]);
        ...
        if (Tools::getValue('condition')) {
            foreach (Tools::getValue('condition') as $category_id => $condition) {
                ...
                foreach ($condition as $type => $condition_ref) {
                    EbayCategoryConditionConfiguration::replace(['id_ebay_profile' => $id_ebay_profile, 'id_condition_ref' => $condition_ref, 'id_category_ref' => $ebay_category_id_ref, 'condition_type' => $type]);
                }
            }
        }
    ...
    public function ajaxProcessActiveCategory()
    {
        ...
        EbayCategoryConfiguration::updateByIdProfileAndIdCategory(Tools::getValue('profile'), Tools::getValue('ebay_category'), $data = ['sync' => $value]);
    ...
    public function ajaxProcessLoadConfigFromCategory()
    {
        if ($id_categori_ps = Tools::getValue('id_category_ps')) {
            if (Module::isInstalled('ebay')) {
                ...
                $category_list = $ebay->getChildCategories($categories, $root_category->id, [], '', Tools::getValue('s'));
            ...
            ORDER BY `level`';
                ...
                foreach ($datas as $category) {
                    ...
                    if (!isset($ref_categories[$category['id_category_ref']])) {
                        ...
                        if ($category['id_category_ref'] != $category['id_category_ref_parent']) {
                            if (!isset($ref_categories[$category['id_category_ref_parent']]['children'])) {
                                $ref_categories[$category['id_category_ref_parent']]['children'] = [];
                            }
                ...
                foreach ($category_config_list as &$category) {
                    ...
                    if ($category['percent']) {
                        ...
                        $category['percent'] = ['sign' => $temp[1], 'value' => $temp[2], 'type' => ($temp[3] == '' ? '€' : $temp[3])];
                    } else {
                        $category['percent'] = ['sign' => '+', 'value' => '0', 'type' => '€'];
                    }
                }
                ...
                if ($category_config_list[$id_categori_ps]['percent']) {
                    ...
                    $percent = ['sign' => $temp['sign'], 'value' => $temp['value'], 'type' => $temp['type']];
                } else {
                    $percent = ['sign' => '', 'value' => '', 'type' => ''];
                }
                $vars = [
                    'tabHelp' => '&id_tab=7',
                    '_path' => $ebay->getPath(),
    ...
    public function ajaxProcessLoadAjaxCategories()
    {
        ...
        $root_category = Category::getRootCategory();
        $categories = Category::getCategories(Tools::getValue('id_lang'));
        $category_list = $ebay->getChildCategories($categories, $root_category->id, [], '', Tools::getValue('s'), (int) Tools::getValue('profile'));
    ...
    public function ajaxProcessLoadFormSyncTab()
    {
        ...
        exit($form_ebay_sync_tab->getContent((int) $page_current, $length, $searche, $filter));
    }
